...
class RPC:
    ...
    def _rpc_show_config(
        ...
        val = {
            ...
            'short_allowed':
            config.get('trading_mode', 'spot') != 'spot',
            'stake_currency':
            ...
            'available_capital':
            config.get('available_capital'),
            'max_open_trades': (config.get('max_open_trades', 0) if config.get(
                'max_open_trades', 0) != float('inf') else -1),
            'minimal_roi':
            config['minimal_roi'].copy() if 'minimal_roi' in config else {},
            'stoploss':
            config.get('stoploss'),
            'stoploss_on_exchange':
            config.get('order_types', {}).get('stoploss_on_exchange', False),
            'trailing_stop':
            ...
            config['strategy'],
            'force_entry_enable':
            config.get('force_entry_enable', False),
            'exit_pricing':
            config.get('exit_pricing', {}),
            'entry_pricing':
            config.get('entry_pricing', {}),
            'state':
            str(botstate),
            'runmode':
            config['runmode'].value,
            'position_adjustment_enable':
            config.get('position_adjustment_enable', False),
    ...
    def _rpc_trade_status(self,
        ...
        if not trades:
            ...
            for trade in trades:
                ...
                trade_dict.update(
                    dict(close_profit=trade.close_profit
                         if not trade.is_open else None,
    ...
    def _rpc_status_table(
        ...
        nonspot = self._config.get('trading_mode',
        ...
        if not trades:
            ...
            for trade in trades:
                ...
                try:
                    ...
                    if trade.nr_of_successful_entries > 0:
                        ...
                        profit_str = f'{0.0:.2f}'
                direction_str = (
                    'S' if trade.is_short else 'L') if nonspot else ''
                ...
                active_attempt_side_symbols = [
                    '*' if
                    (oo and oo.ft_order_side == trade.entry_side) else '**'
                    for oo in trade.open_orders
                ]

                # exemple: '*.**.**' trying to enter, exit and exit with 3 different orders
                active_attempt_side_symbols_str = '.'.join(
                    active_attempt_side_symbols)
                ...
                detail_trade = [
                    ...
                    shorten_date(
                        dt_humanize(trade.open_date,
                                    only_distance=True)), profit_str
                ]

                if self._config.get('position_adjustment_enable', False):
                    max_entry_str = ''
                    if self._config.get('max_entry_position_adjustment',
                                        -1) > 0:
            ...
            columns = [
                'ID L/S' if nonspot else 'ID', 'Pair', 'Since', profitcol
            ]
            if self._config.get('position_adjustment_enable', False):
                columns.append('# Entries')
            return trades_list, columns, fiat_profit_sum

    def _rpc_timeunit_profit(self,
                             timescale: int,
                             stake_currency: str,
        ...
        if timeunit == 'weeks':
            ...
            start_date = start_date - timedelta(
                days=start_date.weekday())  # Monday
        if timeunit == 'months':
        ...
        for day in range(0, timescale):
            ...
            trades = Trade.session.execute(
                select(Trade.close_profit_abs).filter(
                    Trade.is_open.is_(False), Trade.close_date >= profitday,
                    Trade.close_date < (profitday + time_offset(1))).order_by(
                        Trade.close_date)).all()
    ...
    def _rpc_trade_history(self,
                           limit: int,
                           offset: int = 0,
                           order_by_id: bool = False) -> Dict:
    ...
    def _rpc_stats(self) -> Dict[str, Any]:
        ...
        wins_dur = sum(dur['wins']) / len(dur['wins']) if len(
            dur['wins']) > 0 else None
        draws_dur = sum(dur['draws']) / len(dur['draws']) if len(
            dur['draws']) > 0 else None
        losses_dur = sum(dur['losses']) / len(dur['losses']) if len(
            dur['losses']) > 0 else None
    ...
    def _rpc_trade_statistics(
        ...
        for trade in trades:
            ...
            if not trade.is_open:
                ...
                if isnan(current_rate):
                    ...
                    profit = trade.calculate_profit(trade.close_rate
                                                    or current_rate)
        ...
        profit_factor = winning_profit / abs(
            losing_profit) if losing_profit else float('inf')
        ...
        if len(trades_df) > 0:
            try:
                (max_drawdown_abs, drawdown_start, drawdown_end, dd_high_val,
                 dd_low_val, max_drawdown) = calculate_max_drawdown(
                     trades_df,
        ...
        return {
            ...
            profit_closed_ratio_sum,
            'profit_closed_ratio':
            profit_closed_ratio_fromstart,
            'profit_closed_percent':
            round(profit_closed_ratio_fromstart * 100, 2),
            'profit_closed_fiat':
            profit_closed_fiat,
            'profit_all_coin':
            profit_all_coin_sum,
            ...
            profit_all_ratio_sum,
            'profit_all_ratio':
            profit_all_ratio_fromstart,
            'profit_all_percent':
            round(profit_all_ratio_fromstart * 100, 2),
            'profit_all_fiat':
            profit_all_fiat,
            'trade_count':
            len(trades),
            ...
            dt_humanize(first_date) if first_date else '',
            'first_trade_timestamp':
            dt_ts_def(first_date, 0),
            'latest_trade_date':
            format_date(last_date),
            'latest_trade_humanized':
            dt_humanize(last_date) if last_date else '',
            'latest_trade_timestamp':
            dt_ts_def(last_date, 0),
            'avg_duration':
            str(timedelta(seconds=sum(durations) / num)).split('.')[0],
            'best_pair':
            best_pair[0] if best_pair else '',
            'best_rate':
            round(best_pair[1] * 100, 2) if best_pair else 0,  # Deprecated
            'best_pair_profit_ratio':
            best_pair[1] if best_pair else 0,
            'winning_trades':
            winning_trades,
            'losing_trades':
            losing_trades,
            ...
            max_drawdown_abs,
            'max_drawdown_start':
            format_date(drawdown_start),
            'max_drawdown_start_timestamp':
            dt_ts_def(drawdown_start),
            'max_drawdown_end':
            format_date(drawdown_end),
            'max_drawdown_end_timestamp':
            dt_ts_def(drawdown_end),
            'drawdown_high':
            dd_high_val,
            'drawdown_low':
            dd_low_val,
            ...
            trading_volume,
            'bot_start_timestamp':
            dt_ts_def(bot_start, 0),
            'bot_start_date':
            format_date(bot_start),
        }

    def __balance_get_est_stake(self, coin: str, stake_currency: str,
                                amount: float, balance: Wallet,
                                tickers) -> Tuple[float, float]:
        est_stake = 0.0
        est_bot_stake = 0.0
        if coin == stake_currency:
            est_stake = balance.total
            if self._config.get('trading_mode',
                                TradingMode.SPOT) != TradingMode.SPOT:
                # in Futures, "total" includes the locked stake, and therefore all positions
                est_stake = balance.free
            est_bot_stake = amount
        else:
            pair = self._freqtrade.exchange.get_valid_pair_combination(
                coin, stake_currency)
            rate: Optional[float] = tickers.get(pair, {}).get('last', None)
            if rate:
                if pair.startswith(
                        stake_currency) and not pair.endswith(stake_currency):
                    rate = 1.0 / rate
    ...
    def _rpc_balance(self, stake_currency: str,
                     fiat_display_currency: str) -> Dict:
        """ Returns current account balance per crypto """
        currencies: List[Dict] = []
        total = 0.0
        total_bot = 0.0
        try:
            tickers: Tickers = self._freqtrade.exchange.get_tickers(
                cached=True)
        except (ExchangeError):
            raise RPCException('Error getting current tickers.')

        open_trades: List[Trade] = Trade.get_open_trades()
        open_assets: Dict[str, Trade] = {
            t.safe_base_currency: t
            for t in open_trades
        }
        self._freqtrade.wallets.update(require_update=False)
        starting_capital = self._freqtrade.wallets.get_starting_balance()
        starting_cap_fiat = self._fiat_converter.convert_amount(
            starting_capital, stake_currency,
            fiat_display_currency) if self._fiat_converter else 0
        coin: str
        balance: Wallet
        for coin, balance in self._freqtrade.wallets.get_all_balances().items(
            ...
            currencies.append({
                ...
                est_stake or 0,
                'est_stake_bot':
                est_stake_bot if is_bot_managed else 0,
                'stake':
                stake_currency,
                'side':
                'long',
        ...
        return {
            ...
            round(starting_capital_ratio * 100, 2),
            'starting_capital_fiat':
            starting_cap_fiat,
            'starting_capital_fiat_ratio':
            starting_cap_fiat_ratio,
            'starting_capital_fiat_pct':
            round(starting_cap_fiat_ratio * 100, 2),
            'trade_count':
            trade_count,
            'note':
            'Simulated balances' if self._freqtrade.config['dry_run'] else ''
        }
    ...
    def __exec_force_exit(self,
        ...
        for oo in trade.open_orders:
            ...
            order = self._freqtrade.exchange.fetch_order(
                oo.order_id, trade.pair)

            if order['side'] == trade.entry_side:
                fully_canceled = self._freqtrade.handle_cancel_enter(
                    trade, order, oo, CANCEL_REASON['FORCE_EXIT'])
                trade_entry_cancelation_res['cancel_state'] = fully_canceled
                trade_entry_cancelation_registry.append(
                    trade_entry_cancelation_res)

            if order['side'] == trade.exit_side:
                # Cancel order - so it is placed anew with a fresh price.
                self._freqtrade.handle_cancel_exit(trade, order, oo,
                                                   CANCEL_REASON['FORCE_EXIT'])

        if all(tocr['cancel_state'] is False
               for tocr in trade_entry_cancelation_registry):
            ...
            self._freqtrade.execute_trade_exit(trade,
                                               current_rate,
                                               exit_check,
    ...
    def _force_entry_validations(self, pair: str, order_side: SignalDirection):
        ...
        if pair not in self._freqtrade.exchange.get_markets(
                tradable_only=True):
            raise RPCException(
                'Symbol does not exist or market is not active.')
        # Check if pair quote currency equals to the stake currency.
        stake_currency = self._freqtrade.config.get('stake_currency')
        if not self._freqtrade.exchange.get_pair_quote_currency(
                pair) == stake_currency:
            raise RPCException(
                f'Wrong pair selected. Only pairs with stake-currency {stake_currency} allowed.'
            )

    def _rpc_force_entry(self,
                         pair: str,
    ...
    def _rpc_cancel_open_order(self, trade_id: int):
        ...
        with self._freqtrade._exit_lock:
            ...
            if not trade.has_open_orders:
                logger.warning(
                    'cancel_open_order: No open order for trade_id.')
    ...
    def _rpc_delete(self, trade_id: int) -> Dict[str, Union[str, int]]:
        ...
        with self._freqtrade._exit_lock:
            ...
            if (self._freqtrade.strategy.order_types.get(
                    'stoploss_on_exchange') and trade.has_open_sl_orders):
            ...
            return {
                ...
                'trade_id': trade_id,
                'result_msg':
                f'Deleted trade {trade_id}. Closed {c_count} open orders.',
                'cancel_order_count': c_count,
            }

    def _rpc_list_custom_data(self, trade_id: int,
    ...
    def _rpc_enter_tag_performance(
            self, pair: Optional[str]) -> List[Dict[str, Any]]:
        """
    ...
    def _rpc_exit_reason_performance(
            self, pair: Optional[str]) -> List[Dict[str, Any]]:
        """
    ...
    def _rpc_mix_tag_performance(self,
    ...
    def _rpc_count(self) -> Dict[str, float]:
        ...
        return {
            ...
            'max': (int(self._freqtrade.config['max_open_trades'])
                    if self._freqtrade.config['max_open_trades']
                    != float('inf') else -1),
            'total_stake':
            sum((trade.open_rate * trade.amount) for trade in trades)
        }
    ...
    def _rpc_add_lock(self, pair: str, until: datetime, reason: Optional[str],
                      side: str) -> PairLock:
    ...
    def _rpc_blacklist_delete(self, delete: List[str]) -> Dict:
        ...
        for pair in delete:
            if pair in self._freqtrade.pairlists.blacklist:
                ...
                errors[pair] = {
                    'error_msg':
                    f"Pair {pair} is not in the current blacklist."
                }
            ...
            for pair in add:
                if pair not in self._freqtrade.pairlists.blacklist:
                    try:
                        expand_pairlist(
                            [pair],
                            self._freqtrade.exchange.get_markets().keys())
                        self._freqtrade.pairlists.blacklist.append(pair)

                    except ValueError:
                        errors[pair] = {
                            'error_msg':
                            f'Pair {pair} is not a valid wildcard.'
                        }
                else:
                    errors[pair] = {
    ...
    def _rpc_get_logs(limit: Optional[int]) -> Dict[str, Any]:
        ...
        records = [[
            format_date(datetime.fromtimestamp(r.created)), r.created * 1000,
            r.name, r.levelname,
        ...
        if has_content:
            ...
            for sig_type in signals.keys():
                if sig_type in dataframe.columns:
                    ...
                    dataframe.loc[mask,
                                  f'_{sig_type}_signal_close'] = dataframe.loc[
                                      mask, 'close']
    ...
    def _rpc_analysed_dataframe(self, pair: str, timeframe: str,
        ...
        _data, last_analyzed = self.__rpc_analysed_dataframe_raw(
            pair, timeframe, limit)
        return RPC._convert_dataframe_to_dict(
            self._freqtrade.config['strategy'], pair, timeframe, _data,
            last_analyzed)
    ...
    def _ws_all_analysed_dataframes(
            self, pairlist: List[str],
            limit: Optional[int]) -> Generator[Dict[str, Any], None, None]:
        """
        ...
        candle_type = self._freqtrade.config.get('candle_type_def',
                                                 CandleType.SPOT)
    ...
    def _rpc_analysed_history_full(config: Config, pair: str, timeframe: str,
        ...
        df_analyzed = trim_dataframe(df_analyzed,
                                     timerange_parsed,
                                     startup_candles=startup_candles)

        return RPC._convert_dataframe_to_dict(strategy.get_strategy_name(),
                                              pair, timeframe,
                                              df_analyzed.copy(), dt_now())

    def _rpc_plot_config(self) -> Dict[str, Any]:
        if (self._freqtrade.strategy.plot_config
    ...
    def health(self) -> Dict[str, Optional[Union[str, int]]]:
        ...
        if last_p is not None:
            res.update({
                ...
                str(last_p),
                "last_process_loc":
                format_date(last_p.astimezone(tzlocal())),
                "last_process_ts":
                int(last_p.timestamp()),
            })

        if (bot_start :=
                KeyValueStore.get_datetime_value(KeyStoreKeys.BOT_START_TIME)):
            res.update({
                "bot_start":
                str(bot_start),
                "bot_start_loc":
                format_date(bot_start.astimezone(tzlocal())),
                "bot_start_ts":
                int(bot_start.timestamp()),
            })
        if (bot_startup :=
                KeyValueStore.get_datetime_value(KeyStoreKeys.STARTUP_TIME)):
            res.update({
                "bot_startup":
                str(bot_startup),
                "bot_startup_loc":
                format_date(bot_startup.astimezone(tzlocal())),
                "bot_startup_ts":
                int(bot_startup.timestamp()),
            })
