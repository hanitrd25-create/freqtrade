...
class TheGreatDivergenceSpot(IStrategy):
    """
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        TheGreatDivergenceSpot.plot_config = (
            PlotConfig()
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            (
                (dataframe[resample('total_bullish_divergences')].shift() > 0)  # Check bullish divergences
                & (dataframe['high'] > dataframe['high'].shift())  # Current high is greater than previous high
                & (
                    (keltner_middleband_check(dataframe) & ema_check(dataframe) & green_candle(dataframe))  # Condition 1
                    | (keltner_middleband_check(dataframe) & green_candle(dataframe))  # Condition 2
                    | (keltner_lowerband_check(dataframe) & ema_check(dataframe))  # Condition 3
                    | keltner_lowerband_check(dataframe)  # Condition 4
                    | (keltner_lowerband_check(dataframe) & green_candle(dataframe))  # Condition 5
                    | (bollinger_lowerband_check(dataframe) & ema_check(dataframe))  # Condition 6
                )
                & two_bands_check(dataframe)  # Custom condition: two bands check
                & bollinger_keltner_check(dataframe)  # Custom condition: bollinger-keltner check
                & ema_cross_check(dataframe)  # Custom condition: EMA cross check
                & (dataframe['volume'] > 0)  # Ensure volume is greater than 0
            ),
            'buy'] = 1  # Set the buy signal
        return dataframe
